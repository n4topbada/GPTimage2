...
`npx`? ????, Codex OAuth? ???? ?, ????? ????? ????, ????, ???? ??, Canvas Mode ?? ???? ?? ??? ? ????. ?? ??? ?? ????? OpenAI API ?? ???? ????.
...
| `IMA2_GENERATED_DIR` | `<packageRoot>/generated` | 생성 이미지 저장 위치 |
...
생성 이미지는 기본적으로 프로젝트/패키지 루트의 `generated/` 폴더에 저장됩니다. 예전 설치 위치에서 이미지를 복구해야 하면 `ima2 doctor`를 실행하고 [예전 이미지 복구 안내](RECOVER_OLD_IMAGES.md)를 확인하세요.
