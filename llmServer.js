...
function responseInputFromTextBody(body) {
...
function buildTextResponseBody(body = {}) {
    const out = {
    ...
    if (body.reasoning !== undefined)
        ...
        out.reasoning = { effort: body.reasoningEffort };
    if (body.text !== undefined && typeof body.text === "object")
        out.text = body.text;
    return out;
}
function chatMessagesFromTextBody(body = {}) {
    if (Array.isArray(body.messages))
...
function buildTextChatBody(body = {}) {
    const out = {
        model: body.model || config.llm.defaultModel,
...
export async function startLlmServer(overrides = {}) {
    ...
    const server = await listenWithPortFallback(app, config.llm.port, {
